...
class Recorder:
    ...
    def writeInfo2File(self):
        ...
        for threadKey in self.cycleInfo.keys():
            ...
            headerText = ["cycleNumber", "liveIndividuals", "deadIndividuals", "newBorn", "newDeath", "breedTimes", "fightTimes", "fightSuccessTimes", "fightPeaceTimes", "fightFailureTimes", "defendTimes", "defendSuccessTimes", "defendPeaceTimes", "defendFailureTimes", "popAvgHungryLevel", "popAvgAge", "popAvgLifespan", "popAvgFightCapability", "popAvgAttackPossibility", "popAvgDefendPossibility", "popAvgTotalBreedingTimes"]
            ...
            for cycleNumber in threadValue.keys():
                ...
                sheet.write(rowNum, 10, cycleInfo.defendTimes)
                sheet.write(rowNum, 11, cycleInfo.defendSuccessTimes)
                sheet.write(rowNum, 12, cycleInfo.defendPeaceTimes)
                sheet.write(rowNum, 13, cycleInfo.defendFailureTimes)
                sheet.write(rowNum, 14, cycleInfo.popAvgHungryLevel)
                sheet.write(rowNum, 15, cycleInfo.popAvgAge)
                sheet.write(rowNum, 16, cycleInfo.popAvgLifespan)
                sheet.write(rowNum, 17, cycleInfo.popAvgFightCapability)
                sheet.write(rowNum, 18, cycleInfo.popAvgAttackPossibility)
                sheet.write(rowNum, 19, cycleInfo.popAvgDefendPossibility)
                sheet.write(rowNum, 20, cycleInfo.popAvgTotalBreedingTimes)
                rowNum += 1
