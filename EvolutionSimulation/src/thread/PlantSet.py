...
class PlantSet(PopulationThread):
    ...
    def __init__(self, plant_count, dreamland: Dreamland):
        ...
        for i in range(0, plant_count):
            plant = Plant()
            plant.coordinateX = random.randint(0, Dreamland.SIZE_X - 1)
            plant.coordinateY = random.randint(0, Dreamland.SIZE_Y - 1)
            # set the slot code in the dreamland
            plant.slotCode = Dreamland.returnSlotCode(plant.coordinateX, plant.coordinateY)
